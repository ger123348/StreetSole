...
<head>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        .bank-option { cursor: pointer; transition: all 0.2s; }
        .bank-option.selected { background: rgba(255,255,255,0.1); border-color: white; }
...
<body class="flex h-screen overflow-hidden">
    ...
    <script>
        ...
        function openCheckout() { if (cart.length === 0) { showToast("Keranjang masih kosong!", false); return; } const modal = document.getElementById('checkoutModal'); checkoutStep = 1; selectedPayment = "transfer"; selectedBank = bankList[0]; selectedMapLocation = null; renderCheckout(); modal.classList.add('active'); setTimeout(() => { initAddressMap(); setupAddressFormListener(); }, 200); }
        function renderCheckout() { const subtotal = cart.reduce((s,i)=>s+(i.price*i.qty),0); const shipping=25000; const discount=50000; const total=subtotal+shipping-discount; const content=document.getElementById('checkoutContent'); content.innerHTML=`<div class="mb-6"><div class="flex items-center gap-2 mb-4"><div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${checkoutStep>=1?'bg-white text-black':'bg-white/10 text-white/30'}">1</div><div class="h-px flex-1 ${checkoutStep>=2?'bg-white':'bg-white/20'}"></div><div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${checkoutStep>=2?'bg-white text-black':'bg-white/10 text-white/30'}">2</div><div class="h-px flex-1 ${checkoutStep>=3?'bg-white':'bg-white/20'}"></div><div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${checkoutStep>=3?'bg-white text-black':'bg-white/10 text-white/30'}">3</div></div><p class="text-center text-xs text-white/40">${checkoutStep===1?'Alamat Pengiriman (Pilih di Map)':checkoutStep===2?'Metode Pembayaran':'Konfirmasi Pesanan'}</p></div>${checkoutStep===1?renderStep1():checkoutStep===2?renderStep2():renderStep3(total)}<div class="flex gap-3 mt-6">${checkoutStep>1?`<button onclick="prevCheckoutStep()" class="flex-1 bg-white/5 border border-white/10 py-3 rounded-xl text-sm hover:bg-white/10 transition">Kembali</button>`:''}<button onclick="${checkoutStep===3?'confirmOrder()':'nextCheckoutStep()'}" class="flex-1 bg-white text-black py-3 rounded-xl font-semibold text-sm hover:bg-white/90 transition">${checkoutStep===3?'Konfirmasi Pesanan':'Lanjutkan'}</button></div>`; }
        ...
        function renderStep2() { return `<div><h3 class="font-semibold text-sm mb-4">Metode Pembayaran</h3><div class="space-y-3"><div class="payment-method-card p-4 rounded-xl ${selectedPayment==='transfer'?'selected border-white':'border-white/10 border'}" onclick="selectPaymentMethod('transfer')"><div class="flex items-center gap-3"><i class="fas fa-university text-xl"></i><div><p class="font-semibold text-sm">Transfer Bank</p><p class="text-xs text-white/40">BCA, Mandiri, BRI, BNI, dan lainnya</p></div></div></div><div class="payment-method-card p-4 rounded-xl ${selectedPayment==='dana'?'selected border-white':'border-white/10 border'}" onclick="selectPaymentMethod('dana')"><div class="flex items-center gap-3"><i class="fab fa-google-pay text-xl"></i><div><p class="font-semibold text-sm">DANA</p><p class="text-xs text-white/40">E-wallet instant</p></div></div></div><div class="payment-method-card p-4 rounded-xl ${selectedPayment==='qris'?'selected border-white':'border-white/10 border'}" onclick="selectPaymentMethod('qris')"><div class="flex items-center gap-3"><i class="fas fa-qrcode text-xl"></i><div><p class="font-semibold text-sm">QRIS</p><p class="text-xs text-white/40">Scan QR untuk bayar</p></div></div></div><div class="payment-method-card p-4 rounded-xl ${selectedPayment==='cod'?'selected border-white':'border-white/10 border'}" onclick="selectPaymentMethod('cod')"><div class="flex items-center gap-3"><i class="fas fa-truck text-xl"></i><div><p class="font-semibold text-sm">COD (Bayar di Tempat)</p><p class="text-xs text-white/40">Bayar saat barang tiba</p></div></div></div></div>${selectedPayment==='transfer'?renderBankSelection():''}</div>`; }
        function renderBankSelection() { return `<div class="mt-4"><p class="text-xs text-white/40 mb-3 uppercase tracking-widest">Pilih Bank</p><div class="grid grid-cols-2 gap-2">${bankList.map(bank => `<div class="bank-option p-3 rounded-xl border ${selectedBank?.id===bank.id?'border-white bg-white/10':'border-white/10 bg-white/5'} flex items-center gap-3 cursor-pointer transition-all hover:bg-white/10" onclick="selectBank('${bank.id}')"><i class="${bank.icon} text-base ${selectedBank?.id===bank.id?'text-white':'text-white/40'}"></i><span class="text-sm font-medium">${bank.name}</span>${selectedBank?.id===bank.id?'<i class="fas fa-check-circle ml-auto text-emerald-400 text-xs"></i>':''}</div>`).join('')}</div></div>`; }
        function renderStep3(total) { let paymentDetailHtml=""; if(selectedPayment==='transfer' && selectedBank){ paymentDetailHtml=`<div class="mt-4 p-4 rounded-xl bg-white/5"><p class="text-sm font-semibold mb-2">Transfer ke ${selectedBank.name}</p><p class="text-lg font-mono tracking-wider text-center">${selectedBank.vaNumber}</p><p class="text-xs text-white/40 text-center mt-2">Total yang harus ditransfer: <span class="text-white font-bold">Rp ${total.toLocaleString('id-ID')}</span></p></div>`; }else if(selectedPayment==='dana'){ paymentDetailHtml=`<div class="mt-4 p-4 rounded-xl bg-white/5 text-center"><i class="fab fa-google-pay text-3xl text-white/40 mb-2"></i><p class="text-lg font-mono">0857 1234 5678</p><p class="text-xs text-white/40 mt-2">Transfer ke nomor DANA di atas<br>Total: Rp ${total.toLocaleString('id-ID')}</p></div>`; }else if(selectedPayment==='qris'){ paymentDetailHtml=`<div class="mt-4 p-4 rounded-xl bg-white/5 text-center"><div class="w-32 h-32 bg-white mx-auto rounded-xl flex items-center justify-center"><i class="fas fa-qrcode text-6xl text-black"></i></div><p class="text-xs text-white/40 mt-2">Scan QRIS untuk membayar<br>Total: Rp ${total.toLocaleString('id-ID')}</p></div>`; }else if(selectedPayment==='cod'){ paymentDetailHtml=`<div class="mt-4 p-4 rounded-xl bg-white/5 text-center"><i class="fas fa-truck text-2xl text-white/40 mb-2"></i><p class="text-sm">Bayar tunai saat barang sampai</p><p class="text-xs text-white/40 mt-1">Total: Rp ${total.toLocaleString('id-ID')}</p></div>`; } return `<div class="space-y-4"><h3 class="font-semibold text-sm">Detail Pesanan</h3>${cart.map(item=>`<div class="flex justify-between text-sm"><span>${item.name}(${item.size}) x${item.qty}</span><span>Rp ${(item.price*item.qty).toLocaleString('id-ID')}</span></div>`).join('')}<div class="border-t border-white/10 pt-3"><div class="flex justify-between text-xs text-white/50"><span>Subtotal</span><span>Rp ${cart.reduce((s,i)=>s+(i.price*i.qty),0).toLocaleString('id-ID')}</span></div><div class="flex justify-between text-xs text-white/50"><span>Ongkir</span><span>Rp 25.000</span></div><div class="flex justify-between text-xs text-emerald-400"><span>Diskon</span><span>- Rp 50.000</span></div><div class="flex justify-between font-bold pt-2"><span>Total</span><span>Rp ${total.toLocaleString('id-ID')}</span></div></div>${paymentDetailHtml}</div>`; }
        ...
        function nextCheckoutStep() { if(checkoutStep===1){ const firstName=document.getElementById('firstName')?.value; if(!firstName){showToast("Masukkan nama depan",false); return;} if(!selectedMapLocation){showToast("Pilih lokasi di peta terlebih dahulu",false); return;} shippingData={firstName:document.getElementById('firstName').value,lastName:document.getElementById('lastName').value,phone:document.getElementById('phone').value,address:document.getElementById('address').value,city:document.getElementById('city').value,zip:document.getElementById('zip').value}; } if(checkoutStep===2 && selectedPayment==='transfer' && !selectedBank){ showToast("Pilih bank terlebih dahulu",false); return; } checkoutStep++; renderCheckout(); playSound('success'); }
        ...
        function confirmOrder() {
            ...
            .catch(err => { console.error(err); showToast('❌ Gagal menyimpan pesanan', false); });
        }
